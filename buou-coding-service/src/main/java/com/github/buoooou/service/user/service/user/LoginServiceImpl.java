...
import com.github.buoooou.api.model.vo.user.UserSaveReq;
import com.github.buoooou.service.user.service.LoginService;
import com.github.buoooou.service.user.service.RegisterService;
import com.github.buoooou.service.user.service.UserAiService;
import com.github.buoooou.service.user.service.UserService;
import com.github.buoooou.service.user.service.help.StarNumberHelper;
    ...
    private void registerPreCheck(UserPwdLoginReq loginReq) {
        ...
        if (StringUtils.isNotBlank(starNumber)) {
            if (Boolean.FALSE.equals(starNumberHelper.checkStarNumber(starNumber))) {
                // 星球编号校验不通过，直接抛异常
                throw ExceptionUtil.of(StatusEnum.USER_STAR_NOT_EXISTS, "VIP编号=" + starNumber);
            }
        ...
        if (StringUtils.isNotBlank(invitationCode) && userAiDao.getByInviteCode(invitationCode) == null) {
            // 填写的邀请码不对, 找不到对应的用户
            throw ExceptionUtil.of(StatusEnum.UNEXPECT_ERROR, "非法的邀请码【" + invitationCode + "】");
        }
    }
}
